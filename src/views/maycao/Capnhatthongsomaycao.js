...
import { Table, Button, Modal, Popconfirm, message, Space, Select, Input, Row, Col } from 'antd'
import {
  EditOutlined,
  DeleteOutlined,
  PlusOutlined,
...
const Capnhatthongsomaycao = () => {
  ...
  const exportToExcel = () => {
    try {
      const rowsToExport =
        selectedRowKeys && selectedRowKeys.length > 0 ? selectedRows : filteredData
  ...
  return (
    <div>
      ...
      <Modal
        className="mt-5"
        title={editing ? 'Sửa thông số' : 'Thêm thông số'}
        open={modalVisible}
